...
export const getUserOrders = async (statusFilter = null) => {
  try {
    ...
    if (statusFilter) {
      response = await orderService.get(`/user/orders/?status=${statusFilter}`);
    } else {
      response = await orderService.get(`/user/orders`);
    }
...
export const addOrderItem = async (orderId, menuItemId, quantity = 1) => {
  try {
    const response = await orderService.patch(`/orders/${orderId}/add-item`, {
...
export const updateItemQuantity = async (orderId, orderItemId, quantity) => {
  try {
    const response = await orderService.patch(
      `/orders/${orderId}/update-quantity`,
      {
        order_item_id: orderItemId,
        quantity,
      }
...
export const confirmOrder = async (orderId) => {
  try {
    const response = await orderService.patch(`/orders/${orderId}/confirm`, {});
...
export const getRestaurantOrders = async (
  ...
  try {
    ...
    // Add status filter if provided
    if (statusFilter) {
      response = await orderService.get(
        `/restaurants/${restaurantId}/orders/?status=${statusFilter}`
      );
    } else {
      response = await orderService.get(`/restaurants/${restaurantId}/orders`);
    }
...
export const getOrderById = async (orderId) => {
  try {
    ...
    const response = await orderService.get(`/orders/id/${orderId}`);
