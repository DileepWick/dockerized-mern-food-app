    ...
    try {
      ...
      if (paymentIntent.status === 'succeeded') {
        try {
          ...
          const to = user.email;
          const subject = "Order confirmation from SnapByte"
          const html = `<p>Order : ${currentOrder.orderDetails.order_id} </p><p>Total : ${currentOrder.orderDetails.total_amount} </p>`


          // Send email notification - properly await this async function
          try {
            ...
            // We don't throw here as email failure shouldn't stop the payment process
          }
          ...
          // Handle the error appropriately - maybe set an error state and show user
          setPaymentError(error.message || 'An error occurred during payment processing');
        }
      } else {
        setPaymentError('Payment failed');
        console.error('Payment intent did not succeed:', paymentIntent.status);
      }
    } catch (err) {
