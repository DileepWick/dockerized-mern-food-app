...
import { Loader2, Edit, Save, RefreshCcw, AlertTriangle } from 'lucide-react';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs';
import { getLoggedInUser, isAuthorized } from '../utils/authUtils';
import { restaurantService } from '../utils/axiosInstances';
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch the user's restaurant based on userId
        try {
          // Get all restaurants and filter for the one owned by this user
          const restaurantsResponse = await restaurantService.get(
            '/restaurants'
          );
          const userRestaurant = restaurantsResponse.data.find(
            (restaurant) => restaurant.owner_id === userData.userId
          );
          ...
          if (userRestaurant) {
            ...
            setError(
              'No restaurant found. Please contact support to create one.'
            );
          }
      ...
      <div className='flex flex-col items-center justify-center min-h-screen'>
        <Card className='w-full max-w-2xl'>
          <CardHeader>
            ...
            <CardDescription>
              You don&apos;t have a registered restaurant
            </CardDescription>
          </CardHeader>
          <CardContent>
            <p className='text-center'>
              Please contact an administrator to create a restaurant for your
              account.
            </p>
          </CardContent>
        </Card>
